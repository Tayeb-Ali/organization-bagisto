            ...
            <div class="grid-container">
              ...
              <div class="filter-row-one" id="datagrid-filters">
                ...
                <div class="search-filter">
                  <form method="GET" action="{{ route('admin.company.index') }}" accept-charset="UTF-8" class="" style="display: contents;" role="search">
              ...
              <table class="table">
                <thead>
                    <tr>
                        <th>#</th><th>{{ trans('organization::app.company.fields.description') }}</th><th>{{ trans('organization::app.company.fields.currency') }}</th><th>{{ trans('organization::app.company.fields.status') }}</th><th>Actions</th>
                    </tr>
                </thead>
                ...
                @foreach($company as $item)
                    <tr>
                        ...
                        <td>{{ $item->description }}</td><td>{{ $item->currency }}</td><td>{{ $item->status }}</td>
                        <td class="actions" style="white-space: nowrap; width: 100px;">
                          <div class="action">
                            <a href="{{ route('admin.company.show', $item->company_id) }}" title="View Company">
                              <button style="border: none; background-color: inherit;"><span class="icon eye-icon"></span></button>
                            </a>
                            <a href="{{ route('admin.company.edit', $item->company_id) }}" title="Edit Company">
                              <button style="border: none; background-color: inherit;"><span class="icon pencil-lg-icon"></span></button>
                            </a>

                            <form method="POST" action="{{ route('admin.company.delete', $item->company_id) }}" accept-charset="UTF-8" style="display:inline">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
                                <button type="submit" style="border: none; background-color: inherit;" title="Delete Company" onclick="return confirm(&quot;Confirm delete?&quot;)">
