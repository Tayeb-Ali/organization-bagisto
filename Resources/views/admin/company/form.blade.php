        ...
        <div class="row">
            <div class="control-group col col-6 {{ $errors->has('status') ? 'has-error' : ''}}">
                ...
                <select id="status" name="status" class="form-control control" data-role="control">
                    <option value="1" {{$company->status == 1 ? 'selected' : '' }}>{{__('organization::app.status.Inactive')}}</option>
                    <option value="2" {{$company->status ==2 ? 'selected' : '' }}>{{__('organization::app.status.Active')}}</option>
                </select>
