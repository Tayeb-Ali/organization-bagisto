            ...
            <div class="grid-container">
              ...
              <div class="filter-row-one" id="datagrid-filters">
                ...
                <div class="search-filter">
                  <form method="GET" action="{{ route('admin.client.index') }}" accept-charset="UTF-8" class="" style="display: contents;" role="search">
              ...
              <table class="table">
                <thead>
                    <tr>
                        <th>#</th><th>{{ trans('organization::app.client.fields.company_id') }}</th><th>{{ trans('organization::app.client.fields.group_id') }}</th><th>{{ trans('organization::app.client.fields.cust_type') }}</th><th>Actions</th>
                    </tr>
                </thead>
                ...
                @foreach($client as $item)
                    <tr>
                        ...
                        <td>{{ $item->company->description }}</td><td>{{ $item->group_id }}</td><td>{{ $item->cust_type }}</td>
                        <td class="actions" style="white-space: nowrap; width: 100px;">
                          <div class="action">
                            <a href="{{ route('admin.client.show', $item->client_id) }}" title="View Client">
                              <button style="border: none; background-color: inherit;"><span class="icon eye-icon"></span></button>
                            </a>
                            <a href="{{ route('admin.client.edit', $item->client_id) }}" title="Edit Client">
                              <button style="border: none; background-color: inherit;"><span class="icon pencil-lg-icon"></span></button>
                            </a>

                            <form method="POST" action="{{ route('admin.client.delete', $item->client_id) }}" accept-charset="UTF-8" style="display:inline">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
